  ...
  'use strict';
    ...
    */
    var WordFindGame = function() {
        ...
        var wordList, colors, fontSize = 0;
        ...
        */
        var drawPuzzle = function (el, puzzle, screenWidth) {
            var output = '';
            
            fontSize = Math.floor( screenWidth * 0.60 );
            // var paddingTop = Math.floor( fontSize * 0.50 );
            // for each row in the puzzle
            for (var i = 0, height = puzzle.length; i < height; i++) {
                // append a div to represent a row in the puzzle
                var row = puzzle[i];
                var style = 'style="width: ' + screenWidth 
                        + 'px; height: ' + screenWidth 
                        // + 'px; padding-top: ' + paddingTop 
                        + 'px; font-size: ' + fontSize + 'px;" ';
                // console.log(screenWidth, row.length);
                output += '<div style="height: ' + screenWidth + 'px;">';
                // for each element in that row
                for (var j = 0, width = row.length; j < width; j++) {
                    // append our button with the appropriate class
                    output += '<button class="puzzleSquare" '+ style +' x="' + j 
                        + '" y="' + i + '">';
                    output += row[j] || '&nbsp;';
                    output += '</button>';
                }
    ...
    */
    ...
    // Game state
    var startSquare, selectedSquares = [], curOrientation, curWord = '', onComplete;
    ...
    var select = function (target) {
      ...
      var backTo;
        for (var i = 0, len = selectedSquares.length; i < len; i++) {
            // console.log(1, selectedSquares[i], target);
    ...
    */
    var endTurn = function () {
        ...
        for (var i = 0, len = wordList.length; i < len; i++) {
            ...
            // console.log("done word: ", i, len);

            var word = wordList[i];
            if (word === curWord) {
                // $('.selected').addClass('found');
                var colorPos = Math.floor(Math.random() * colors.length);
                var random_color = colors[colorPos];
                colors.splice(colorPos, 1);
                // wordList.splice(i, 1);
                // console.log("random_color", random_color);
                $('.selected').css('background-color', random_color);
                $('.selected').css('color', 'white');
                

                // console.log("done word: ", curWord);
                len = wordList.length;
                $('.' + curWord).addClass('wordFound');
        
                if (wordList.length === 0) {
                    $('.puzzleSquare').addClass('complete');
                    if(onComplete)onComplete(curWord, true);
                }else{
                    if(onComplete)onComplete(word, false);
                }
            }
    ...
    */
    ...
    return {
      ...
      */
    create: function(words, oldPuzzle, puzzleEl, wordsEl, options, width, availableColors) {

        colors = availableColors.slice();
        wordList = [];
        
        wordList = words.slice(0).sort();


        var puzzle = oldPuzzle ? oldPuzzle : wordfind.newPuzzle(wordList, options);
        

        onComplete = options.onComplete;
        // draw out all of the words
        drawPuzzle(puzzleEl, puzzle, width/options.width);
